...
var favoritesURLRegex = /^https?:\/\/(?:[^\.]+\.)?imgur\.com\/account\/favorites/;
...
function addCSS(tabId){
	chrome.tabs.insertCSS(tabId, { file: "/styles/organizedFavorites.css" });
}
...
function updateFavorites(tabId){
	addJavascriptLibraries(tabId);
	addCSS(tabId);
	chrome.tabs.executeScript(tabId, { file: "updateFavorites.js" });
}
	...
	chrome.tabs.query({
		...
		_.each(tabs, function(tab){
			if (galleryURLRegex.test(tab.url)) {
				...
				updatePost(tab.id);
			} else if (favoritesURLRegex.test(tab.url)) {
				/* The URL is for the account's favorites. */
				updateFavorites(tab.id);
			}
		});
	});
}
...
chrome.runtime.onMessage.addListener(function(request, sender) {
	if (request.action == "loadFavoritesOrganizer") {
		queryTabs();
	}
});
